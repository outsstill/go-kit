...
func LimitIPGin(l *Limiter, limit string) gin.HandlerFunc {
	return func(c *gin.Context) {
		// 针对 IP 限流
		key := l.GetKeyIP(c)
		if ok := limitHandler(c, l, key, limit); !ok {
			return
		}
		c.Next()
	}
...
func limitHandler(c *gin.Context, l *Limiter, key string, limit string) bool {

	// 获取超额的情况
	rate, err := l.CheckRate(c, key, limit)
	if err != nil {
		logger.LogIf(err)
		response.Abort500(c)
		return false
	...
	if rate.Reached {
		...
		response.Abort(c, http.StatusTooManyRequests, "接口请求太频繁")
		return false
	}
